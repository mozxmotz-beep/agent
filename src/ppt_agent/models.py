...
class DeckOutline(BaseModel):
    ...
    @field_validator("narrative_flow")
    @classmethod
    def validate_narrative_flow(cls, flow: list[str]) -> list[str]:
        cleaned = [item.strip() for item in flow if item.strip()]
        if len(cleaned) < 3:
            raise ValueError("narrative_flow requires at least 3 stages.")
        return cleaned
